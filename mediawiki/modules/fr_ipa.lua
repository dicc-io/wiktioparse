...
function export.convertToIPA(text, pos, do_debug, no_test_new_module)
	...
	-- 		else
	-- 			text = rsub(text, "%f[%a]" .. require("Module:xc").pattern_escape(from) .. "%f[%A]", to)
	-- 		end
	-- 	end
	-- end
	...
	-- etc.).
	text = rsub(text, '([' .. word_begin .. vowel .. ']h?)[eæ]x(' .. vowel_c .. ')', '%1egz%2')
	text = rsub(text, '(' .. word_begin_c .. 'in)[eæ]x(' .. vowel_c .. ')', '%1egz%2')
